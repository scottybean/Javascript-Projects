...
function getVegetables(runningTotal,text1) {
    ...
    var vegetablesArray = document.getElementsByClassName("veggies");
    for (var k = 0; k < vegetablesArray.length; k++) {
        if (vegetablesArray[k].checked) {
            selectedVegetable.push(vegetablesArray[k].value);
            console.log("selected topping item: ("+vegetablesArray[k].value+")");
